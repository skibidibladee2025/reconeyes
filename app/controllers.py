...
def scrape_hacked_camera():
    try:
        r = http_get('https://hacked.camera/api/cameras')
        if r.status_code != 200:
            print("Failed to fetch from hacked.camera API")
            return
        ...
        for cam in cameras:
            markers_list.append({
                ...
                "country_code": cam.get('country_code', ''),  # If provided; otherwise, map from country name
        ...
        print(f"Error scraping hacked.camera: {e}")
